...
def generate_grid_interpolate_image(source, reenacted1, reenacted2, reenacted3, reenacted4):
    num_images = source.shape[0]
    width=256; height = 256
    grid_image = torch.zeros((3, num_images * height, 5 * width))
    for i in range(num_images):
        s = i * height 
        e = s + height 
        grid_image[:, s:e, :width] = source[i, :, :, :]
        grid_image[:, s:e, width:2*width] = reenacted1[i, :, :, :]
        grid_image[:, s:e, 2*width:3*width] = reenacted2[i, :, :, :]
        grid_image[:, s:e, 3*width:4*width] = reenacted3[i, :, :, :]
        grid_image[:, s:e, 4*width:] = reenacted4[i, :, :, :]
